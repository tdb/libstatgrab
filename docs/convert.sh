...
for i in "$@"; do
	...
	docbook2x-man -N --no-links --string-param header-3="`git log -1 --format='%ad' --date=short $i`" --to-stdout $i | man --nj --nh -l - | col -b > $i.original
	/home/cur/tdb/.gem/ruby/2.5.0/bin/asciidoctor -a docdate="`git log -1 --format='%ad' --date=short $i`" -b manpage -o - $i.adoc | man --nj --nh -l - | col -b > $i.new
